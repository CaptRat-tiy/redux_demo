...
test('SHIRT@ADD: add red sm to an empty state', t => {
  ...
  t.deepEqual(reducer(startingState, action),
  expectedResult)
  t.end()
})
...
test('SHIRT@ADD: add second red sm to a state with one red sm', t => {
  const starting = [{
    id: 1,
    color: 'red',
    size: 'sm',
...
//REMOVE
test('SHIRT@REMOVE: remove red sm to a state with two shirts', (t) => {
  const startingState = [{
    id: 2,
    color: 'red',
    size: 'sm',
